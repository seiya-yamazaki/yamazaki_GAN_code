...
import sys
sys.path.append("../")
from networks.discriminater.disciminater_LSTM import Discriminator_LSTM
...
class BirdAgent():
    ...
    def __init__(self,  g_units=32, d_units=32, songsize=64):
        self.gen = Generator(n_input=1, n_hidden=g_units, n_output=1)
        self.dis = Discriminator_LSTM(n_input=1, n_hidden=d_units, n_output=1)
        self.opt_gen = optimizers.Adam()
        self.opt_dis = optimizers.Adam()
    ...
    def dis_song(self, song):
        self.dis.reset_state()
        it = song.shape[1]
        for i in xrange(it):
            ret_dis = self.dis(song[:, i])
